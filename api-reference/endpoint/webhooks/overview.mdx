...
Scoped to your account — you only receive deliveries for jobs you started.
...
Broadcast events from [status.generect.com](https://status.generect.com) — subscribe to pipe Generect incidents into your own alerting instead of polling the status page.

| Event | Fired when |
| --- | --- |
| `status.incident.created` | An incident is opened |
| `status.incident.updated` | A new update is posted on an open incident |
| `status.incident.resolved` | An incident is resolved |
| `status.maintenance.started` | Scheduled maintenance starts |
| `status.maintenance.completed` | Scheduled maintenance completes |

Incident deliveries carry `data.incident` (slug, title, status, severity, body, affected `components`, `started_at` / `resolved_at`, full `updates` history) and `data.update` with the update that triggered the event. Maintenance deliveries carry `data.maintenance` (slug, title, body, status, `scheduled_start` / `scheduled_end`, affected `components`). `meta.status_page_url` points back to the public page.
