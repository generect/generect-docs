...
- **Bulk jobs** (enrich bulk, email find bulk, phone find bulk): up to **50 items** per request; a larger array is rejected with `400`.
...
Responses do not currently include `X-RateLimit-*` or `Retry-After` headers — on a `429`, apply exponential backoff (see [Errors](/api-reference/errors)).
