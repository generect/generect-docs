...
<AccordionGroup>
  <Accordion title="5. Search leads by ICP">
    ...
    **Request field mapping:**
    ...
    | `changed_jobs`, `posted_on_linkedin`, `mentioned_in_news`, `groups`, `schools`, `linkedin_filter_link`, `years_in_company`, `years_in_position`, `past_company_names` | same names | Realtime mode only — no longer accepted in `database` mode. |

    **Response:** same shape as [Enrich lead](#enrich) per item, now under `data.leads[]`. `amount` → `meta.amount_charged` (positive now); `status` removed.
  ...
  <Accordion title="7. Search companies by ICP">
    ...
    **Request** — most filters keep their name: `company_types`, `headcounts`, `industries`, `keywords`, `locations`, `num_of_followers`, `limit_by`, `offset_by`. New: `exclude_industries`, `exclude_locations`, `exclude_ids`, `exclude_domains`, `technologies`, `company_names`, `strict`.
    ...
    **Request field mapping:**
    ...
    | `revenues_range` | — | Removed, no v1 equivalent. |
    | `website_blacklist` | — | Removed, no v1 equivalent. |

    **Response:** same shape as [Enrich company](#enrich) per item, now under `data.companies[]`. `amount` → `meta.amount_charged` (positive now); `time_cached` removed.
...
<AccordionGroup>
  <Accordion title="9. Enrich company by link">
    |  | Old | New |
    | --- | --- | --- |
    | Method | `POST` | `POST` |
    ...
    **Response:** same field set as before (company name, domain, website, industry, description, headcount, headcount range, founded year, company type, LinkedIn URL, headquarters/country), now nested under `data`, plus a new `id` field and `meta.amount_charged`.

    | Old field | New field | Notes |
    | --- | --- | --- |
    | `linkedin_id` | — | Removed — use the new `id` field instead. |
    | `headcount_exact` | `headcount` | Renamed. |
    | `linkedin_link` | `linkedin_url` | Renamed. |
    | `hq_city`, `hq_state`, `hq_country` | `headquarters` | Collapsed into one formatted string, e.g. `"Redmond, WA, US"`. |
    | — | `id` | New: Generect internal company identifier. |
    ...
    **Example — before / after:**
    ...
    All fields stay top-level inside `data` — unlike the lead endpoint below, nothing here moves into a sub-object. Only the wrapper and the new `id`/`meta.amount_charged` fields are new.
  ...
  <Accordion title="10. Enrich lead by LinkedIn URL">
    ...
    **Response field mapping:**
    ...
    | Old field | New field | Notes |
    | --- | --- | --- |
    | `first_name` | `data.first_name` | Unchanged. |
    | `last_name` | `data.last_name` | Unchanged. |
    | `job_title` | `data.title` | Renamed. |
    | `headline` | `data.headline` | Unchanged, when available per profile. |
    | `linkedin_url` | `data.linkedin_url` | Unchanged. |
    | `location` | `data.location` | Unchanged. |
    ...
    | `company_industry` | `data.company.industry` | Now nested inside `company` object. |
    ...
    **Example — before / after:**
    ...
    All top-level lead fields stay the same, except the three company fields (`company_name`, `company_website`, `company_industry`) — those moved into `data.company.{name, domain, industry}`, plus a new `company.id` and top-level `meta.amount_charged`.
...
<AccordionGroup>
  ...
  <Accordion title="12. Validate emails">
    ...
    **Example — before / after:**
    ...
    {
      "data": [
        { "email": "jkennedy@platformscience.com", "result": "valid", "exist": "yes", "catch_all": false, "error": null }
      ],
