...
<Tip>
  Not ready to spend anything? Create a **test key** instead and run the same three calls against the same URLs. Same shapes, same speed, same reported prices, nothing charged. See [Test mode](/api-reference/test-mode).
</Tip>
...
Same filters against [`/search/database/leads/`](/api-reference/endpoint/search/database-leads) — sub-second, billed per returned result ($0.02 each, see [Pricing](/billing/pricing)). Every response tells you what it cost in `meta.amount_charged`.
