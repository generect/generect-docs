...
<Card title="Create a test key" icon="key" href="https://beta.generect.com/settings/api">
  Settings → API → **New key** → choose **Test**. Test keys start with `test_` and stay visible on that page, so you can copy one whenever you need it.
</Card>
...
Test mode honours the `limit_by` you send, up to **500 rows per response**. The live endpoints go higher, but every fixture row is generated through the real serializers, so a page has a real cost.

Ask for more than 500 and you get 500, with `results_count` still reporting the full population so your pagination logic has something to page through. If your integration tests assert on the page size, assert `<=` rather than `==` above that number.
