...
| Profile Resolve | `POST /profile/resolve/` (+ `/bulk/`) | **\$0.0005** | per **resolved** profile |
...
<Note>**Profile Resolve** is priced apart from the cached search & enrich rate because it is a single identity lookup, not a full record: it returns the real profile URL, name, headline and photos, and nothing else. Like Preview, it does **not** move with your tier — it stays at \$0.0005 per resolved profile on every tier. A reference that does not resolve is free, and duplicate references in one bulk call are billed per row.</Note>
