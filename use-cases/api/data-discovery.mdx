...
### 1\. SaaS platform displays search results to users for \$0.002 a result (Preview API)
...
SaaS platforms want to show search results to their users, but paying the full enrichment price for every displayed lead — before the user decides to buy contact data — makes "browse → decide → reveal" flows too expensive to build.

**What Generect enables**

Preview API returns masked cached lead data, such as name, title, company, location, industry, and seniority, with a unique lead ID. Browsing costs \$0.002 per lead shown — a tenth of the database enrichment price — and the full price is paid only when a user reveals a profile or contact data.

**How it works**

1. Your app sends ICP filters to `POST /preview/leads/`
2. Generect returns masked cached data: name, title, company, location, industry, seniority, and lead ID
3. Your users browse and select leads
4. Call `POST /enrich/database/lead/` with `id` for selected leads
5. Pay \$0.002 per lead shown in step 2, and the enrichment price only for the ones your users select
...
- Pay only for actionable data (email, phone, or full profile)
...
`/search/database/company-leads/` or `POST /search/realtime/company-leads/` returns companies matching your ICP together with their key contacts in a single request.
