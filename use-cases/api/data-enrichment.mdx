...
One endpoint, multiple input options: Generect ID, LinkedIn URL, or email. No data found \= no charge.

**How it works**

1. Take lead\_id from Preview results
2. Call `POST /enrich/database/lead/` or `POST /enrich/realtime/lead/`
3. Get full profile: linkedin\_url, work history, education, skills, company details
4. If lead not found — 0 credits charged

**Input data**

Generect ID, OR LinkedIn URL, OR email (reverse lookup)
...
1. Call `POST /enrich/realtime/lead/` with:
   ```json
   {
...
1. Pass company domain (e.g., `microsoft.com`) to `POST /enrich/database/company/`
...
`/email/find/` gets an email by `lead_id` from Preview/Search or by `first_name` + `last_name` + `domain`.

**How it works**

1. Take the lead ID from Preview/Search results
2. Call `POST /email/find/` with:
   ```json
   {
...
2. Get phone number with type, country, and status
